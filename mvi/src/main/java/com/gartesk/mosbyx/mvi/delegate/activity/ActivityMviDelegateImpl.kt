 ...
 */
package com.gartesk.mosbyx.mvi.delegate.activity
...
import com.gartesk.mosbyx.mvi.presentermanager.PresenterManager
import com.gartesk.mosbyx.mvi.shouldRetainPresenterInstance
...
constructor(
	...
	companion object {
		...
		private const val KEY_MOSBY_VIEW_ID = "com.gartesk.mosbyx.activity.mvi.id"
	}
	...
	override fun onStart() {
		...
		if (currentMosbyViewId == null) {
			...
			if (DEBUG) {
				Log.d(
					DEBUG_TAG, "New Presenter instance created: $presenter " +
			...
			if (presenter == null) {
				...
				if (DEBUG) {
					Log.d(
						DEBUG_TAG, "No Presenter instance found in cache, although " +
	...
	override fun onStop() {
		...
		if (DEBUG) {
			Log.d(
				DEBUG_TAG, "Detached MviView from Presenter. " +
